...
${
  gameDetails?.odds?.home.spread == null
    ? `No spread line is available for this game, so an against-the-spread pick cannot be made — do your best anyway; predict the margin you genuinely expect, and it will be discarded if there's truly no line to compare it against.`
    : `The spread line you need to beat: the home team (${match.home}) is ${
        gameDetails.odds.home.spread < 0
          ? `favored by ${Math.abs(gameDetails.odds.home.spread)} points — for ${match.home} to "cover," they must win by MORE than ${Math.abs(gameDetails.odds.home.spread)} points, not just win outright`
          : `an underdog getting ${gameDetails.odds.home.spread} points — for ${match.home} to "cover," they can lose by fewer than ${gameDetails.odds.home.spread} points, tie, or win outright`
      }.`
}
...
Market Odds (repeated for emphasis — this is the line your margin will be compared against)
====================================
