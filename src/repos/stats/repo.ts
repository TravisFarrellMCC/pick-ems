...
const SOURCES = (season: number): { [key in StatType]: string } => ({
...
export class TeamStatsRepo {
  ...
  private static async fetchAll(): Promise<TeamStats[]> {
    const season = await TeamStatsRepo.resolveSeason();
    const teamStats: TeamStats[] = [];

    for (const type of Object.values(StatType)) {
      const stats = await TeamStatsRepo.fetchType(type, season);
  ...
  /**
   * Decides which season's stats to use.
   *
   * CONFIG.NFL_SEASON is "this year's" season (e.g. the season that starts
   * in September), but ESPN has no data for it until games have actually
   * been played. Early in the year (offseason/preseason, or the first
   * couple weeks of the regular season) that page renders "No Data
   * Available", which would silently produce empty/garbage stats. In that
   * case we fall back to the most recently completed season instead of a
   * blank slate.
   *
   * @returns {Promise<number>} The season to fetch stats for.
   */
  private static async resolveSeason(): Promise<number> {
    const season = CONFIG.NFL_SEASON;
    const hasData = await TeamStatsRepo.hasStatsData(season);
    return hasData ? season : season - 1;
  }

  /**
   * Checks whether ESPN has any team stats published for the given season
   * yet, by probing the offense stats page (if one stat page has no data,
   * none of them will).
   *
   * @param season The season to check.
   * @returns {Promise<boolean>} Whether stats data exists for the season.
   */
  private static async hasStatsData(season: number): Promise<boolean> {
    const page = await navigateTo(SOURCES(season)[StatType.OFFENSE], WAIT_FOR);
    const tables = await page.locator(STATS_TABLE).all();
   ...
   * @param season The season to scrape stats for.
   * @returns {Promise<TeamStats[]>} The stats of the given type for all teams.
   */
  private static async fetchType(
    type: StatType,
    season: number,
  ): Promise<TeamStats[]> {
    const page = await navigateTo(SOURCES(season)[type], WAIT_FOR);
